<?php 
  ...
  session_start();
  $id = $_GET['id'];
  $sql = "SELECT * FROM `words` WHERE `text_id` = '".$id."' ";
  $result = $conn->query($sql);
  $row = $result->fetch_assoc();
  // $sql = "SELECT words.text_row, words.word, tags.text_row, tags.tag 
  //         FROM words 
  //         LEFT JOIN tags
  //         ON words.text_row = tags.text_row 
  //         WHERE words.text_id = '".$id."' ";
  //         $result = $conn->query($sql);
          // $row = $result->fetch_assoc();
      ...
      <!-- /.card-header -->
        <div class="card-body">
          <table id="dataTable" class="table table-bordered table-striped">
            <thead>
            <h5 class="card-title">ผลลัพธ์ที่ได้จากระบบตัดคํา ตัดข้อมูลพร้อมกำกับคำ ความคิดเห็นที่..<?php echo $id; ?></h5>
